...
public:
    constexpr HubLabelCalculator(const Graph &graph) noexcept
        : graph_(graph), in_labels_(graph_.numberOfNodes()), out_labels_(graph_.numberOfNodes()) {}
...
private:
        ...
        -> std::vector<HubLabelLookup::HubType>
    {
        ...
        for(const auto edge_id : graph_.getForwardEdgeIDsOf(node)) {
            ...
            std::transform(std::begin(trg_out_hubs), std::end(trg_out_hubs),
                           std::back_inserter(hubs), [&](const auto hub) {
    ...
    constexpr auto
    pruneOutLabels(common::NodeID node,
        ...
        -> std::vector<HubLabelLookup::HubType>
    {
        ...
        for(const auto edge_id : graph_.getBackwardEdgeIDsOf(node)) {
            ...
            std::transform(std::begin(trg_in_hubs), std::end(trg_in_hubs),
                           std::back_inserter(hubs), [&](const auto hub) {
                               const auto [hub_node, hub_dist] = hub;
                               return std::pair{hub_node, hub_dist + weight};
                           });
    ...
    pruneInLabels(common::NodeID node,
                  std::vector<HubLabelLookup::HubType> &labels) const noexcept
        -> void
    {
